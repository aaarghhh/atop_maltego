...
class MaltegoEntity(object):
    ...
    def addProperty(
        self, fieldName=None, displayName=None, matchingRule=False, value=None
    ):
    ...
    def returnEntity(self):
        ...
        if len(self.displayInformation) > 0:
            ...
            for i in range(len(self.displayInformation)):
                r += (
                    '<Label Name="'
                    + self.displayInformation[i][0]
                    + '" Type="text/html"><![CDATA['
                    + str(self.displayInformation[i][1])
                    + "]]></Label>"
                )
            r += "</DisplayInformation>"
        if len(self.additionalFields) > 0:
            ...
            for i in range(len(self.additionalFields)):
                if str(self.additionalFields[i][2]) != "strict":
                    r += (
                        '<Field Name="'
                        + str(self.additionalFields[i][0])
                        + '" DisplayName="'
                        + str(self.additionalFields[i][1])
                        + '">'
                        + str(self.additionalFields[i][3])
                        + "</Field>"
                    )
                else:
                    r += (
                        '<Field MatchingRule="'
                        + str(self.additionalFields[i][2])
                        + '" Name="'
                        + str(self.additionalFields[i][0])
                        + '" DisplayName="'
                        + str(self.additionalFields[i][1])
