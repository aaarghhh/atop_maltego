...
def scan_ton():
    ...
    if current_parser.address != "":
        extra_attributes = []
        extra_attributes.append(["balance", "Balance", "balance",
                                 str(round(int(current_parser.info["result"]["balance"]) / 1000000000, 2))])
        if current_parser.owner_name != "":
            extra_attributes.append(["nickname", "Nickname", "nickname",
                                     current_parser.owner_name])

        entity= MaltegoEntity("atop.TONaddress",current_parser.address,extra_attributes)
        trx.entities.append(entity)
        #addr = trx.addEntity("atop.TONaddress", current_parser.address)
        eee="###TODO handle name "
        #addr=trx.addEntity("atop.TONaddress", website["phoneNumber"])
        ...
        if "data" in current_parser.nfts.keys():
            if "nftItemsByOwner" in current_parser.nfts["data"].keys():
                for item in current_parser.nfts["data"]["nftItemsByOwner"]["items"]:
                    extra_attributes = []
                    extra_attributes.append(["address", "Address", "address",
                                             item["address"]])
                    special = False
                    entity = None
                    ...
                    if "collection" in item.keys() and item["collection"]:
                        if "Usernames" in item["collection"]["name"]:
                            entity = MaltegoEntity("atop.TONnickname", item["name"], extra_attributes)
                            special = True
                        if "Numbers" in item["collection"]["name"]:
                            entity = MaltegoEntity("atop.TONtelephonenumber", item["name"], extra_attributes)
                            special = True
                        if "TON DNS" in item["collection"]["name"]:
                            entity = MaltegoEntity("atop.TONdomain", item["name"], extra_attributes)
                            special = True
                    if not special:
                        entity = MaltegoEntity("atop.TONnft", item["name"], extra_attributes)
                    trx.entities.append(entity)
        ...
        if "data" in current_parser.ens_detail.keys():
            if "domains" in current_parser.ens_detail["data"].keys():
                if len(current_parser.ens_detail["data"]["domains"])>0:
                    entity = MaltegoEntity("maltego.ETHAddress", current_parser.ens_detail["data"]["domains"][0]["owner"]["id"])
                    trx.entities.append(entity)
                    entity = MaltegoEntity("atop.ENSdomain",
